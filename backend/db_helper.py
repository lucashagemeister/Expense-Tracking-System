...
def fetch_expense_summary_by_category(start_date, end_date):
    logger.info(f"fetch_expense_summary_by_category called with start: {start_date} end: {end_date}")
    with get_db_cursor() as cursor:
        cursor.execute(
...
def fetch_expense_summary_by_month():
    ...
    with get_db_cursor() as cursor:
        cursor.execute(
            '''
                SELECT DATE_FORMAT(expense_date, '%M %Y') as Month, SUM(amount) as total
